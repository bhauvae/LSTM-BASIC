...
df = pd.DataFrame(yf.download(tickers=ticker, period="5y")["Close"])
...
def df_to_windowed_df(dataframe, n):
    for i in range(1, n + 1):
        dataframe["Close" + str(-i)] = dataframe["Close"].shift(i)

    dataframe["index"] = (dataframe["Close"].shift(-1) >= dataframe["Close"]).astype(int)
    dataframe.dropna(axis=0, inplace=True)
    
    return dataframe


windowed_df = df_to_windowed_df(df, 3)
